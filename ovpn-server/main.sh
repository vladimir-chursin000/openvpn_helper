...
#EASYRSA_CRL_DAYS_EV
#EASYRSA_KEY_SIZE_EV
#EASYRSA_REQ_COUNTRY_EV
#EASYRSA_REQ_PROVINCE_EV
#EASYRSA_REQ_CITY_EV
#EASYRSA_REQ_ORG_EV
#EASYRSA_REQ_OU_EV
#EASYRSA_REQ_CN_EV
#EASYRSA_REQ_EMAIL_EV
###CFG EXPORT VARS
...
#
export EASYRSA_CRL_DAYS=$EASYRSA_CRL_DAYS_EV;
export EASYRSA_CERT_EXPIRE=$EASYRSA_CERT_EXPIRE_EV;
export EASYRSA_KEY_SIZE=$EASYRSA_KEY_SIZE_EV;
export EASYRSA_REQ_COUNTRY=$EASYRSA_REQ_COUNTRY_EV;
export EASYRSA_REQ_PROVINCE=$EASYRSA_REQ_PROVINCE_EV;
export EASYRSA_REQ_CITY=$EASYRSA_REQ_CITY_EV;
export EASYRSA_REQ_ORG=$EASYRSA_REQ_ORG_EV;
export EASYRSA_REQ_OU=$EASYRSA_REQ_OU_EV;
export EASYRSA_REQ_CN=$EASYRSA_REQ_CN_EV;
export EASYRSA_REQ_EMAIL=$EASYRSA_REQ_EMAIL_EV;
#==================================================================
...
function func_full_setup_vpn_server {
    ...
    echo '###' >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo "# firewall-cmd --permanent --zone=$VPN_SERVER_INT_FIREWALLD_ZONE_S --add-rich-rule='rule family=ipv4 source address=XXX.XXX.XXX.XXX/32 destination address=0.0.0.0/0 masquerade' /// for option 'push \"redirect-gateway def1\"'. XXX.XXX.XXX.XXX/32 - ip from '$VPN_NETWORK_MASQUERADE_SRC_S'" >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo '###' >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo "# firewall-cmd --permanent --zone=$VPN_SERVER_INT_FIREWALLD_ZONE_S --add-rich-rule='rule family=ipv4 source address=XXX.XXX.XXX.XXX/32 destination address=$VPN_NETWORK_MASQUERADE_SRC_S masquerade' /// for access from client to another clients at vpn-network" >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo '###' >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo '# If dnsmasq (or another dns-server) configured at vpn-server' >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo "# firewall-cmd --permanent --zone=$VPN_SERVER_INT_FIREWALLD_ZONE_S --add-rich-rule='rule family=ipv4 source address=$VPN_NETWORK_MASQUERADE_SRC_S port protocol=tcp port=53 accept'" >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo "# firewall-cmd --permanent --zone=$VPN_SERVER_INT_FIREWALLD_ZONE_S --add-rich-rule='rule family=ipv4 source address=$VPN_NETWORK_MASQUERADE_SRC_S port protocol=udp port=53 accept'" >> "$SELF_DIR/readme-firewalld-rules.txt";
    echo '###' >> "$SELF_DIR/readme-firewalld-rules.txt";
    ...
    ###Generate static-ip-plan-file (for restart_scripts)
    echo "###GENERATED by 'full_setup_vpn_server.sh'" > "$SELF_DIR/SET-STATIC-IP-FOR-CLIENTS.txt";
    echo "######" >> "$SELF_DIR/SET-STATIC-IP-FOR-CLIENTS.txt";
    echo "#For set static ip just write 'vpn-user-name,ip-address' and execute 'RESTART_OVPN-$VPN_SERVER_NAME_S.sh'" >> "$SELF_DIR/SET-STATIC-IP-FOR-CLIENTS.txt";
    echo "#user_name,172.16.10.4 (example)" >> "$SELF_DIR/SET-STATIC-IP-FOR-CLIENTS.txt";
    echo "" >> "$SELF_DIR/SET-STATIC-IP-FOR-CLIENTS.txt";
    #
    ...
    echo "\cp \"$SELF_DIR/SET-STATIC-IP-FOR-CLIENTS.txt\" \"$VPN_CONF_DIR_S/$VPN_SERVER_NAME_S-ipp.txt\";" >> "$SELF_DIR/RESTART_OVPN-$VPN_SERVER_NAME_S.sh";
